(function (a) {
    ...
    a.extend(a.fn.modal.Constructor.prototype, {initLinks: function () {
        ...
        this.jQuerylinks = a(c.delegate).find(d).filter(c.filter).each(function (a) {
            b.getUrl(this) === c.href && (c.index = a)
        }), this.jQuerylinks[c.index] || (c.index = 0)
    }, getUrl: function (b) {
        ...
        var a = this.jQueryelement.find(".modal-slideshow");
        ...
        for (e = b.index - b.preloadRange; e < c; e += 1)d = this.jQuerylinks[e], d && e !== b.index && a("<img>").prop("src", this.getUrl(d))
    }, loadImage: function () {
        var a = this, c = this.jQueryelement, d = this.options.index, e = this.getUrl(this.jQuerylinks[d]), f;
        this.abortLoad(), this.stopSlideShow(), c.trigger("beforeLoad"), this._loadingTimeout = window.setTimeout(function () {
            ...
            f.remove()
        }, 3e3), c.find(".modal-title").text(this.jQuerylinks[d].title), c.find(".modal-download").prop("href", e), this._loadingImage = b(e, function (b) {
        ...
        var c = this.jQueryelement, d = a.support.transition && c.hasClass("fade"), e = d ? c.animate : c.css, f = c.find(".modal-image"), g, h;
        ...
        var a = this.options;
        a.index -= 1, a.index < 0 && (a.index = this.jQuerylinks.length - 1), this.loadImage()
    }, next: function () {
        var a = this.options;
        a.index += 1, a.index > this.jQuerylinks.length - 1 && (a.index = 0), this.loadImage()
        ...
        var b = this, c = this.jQueryelement;
        c.find(".modal-image").on("click.modal-gallery", function (c) {
            var d = a(this);
            b.jQuerylinks.length === 1 ? b.hide() : (c.pageX - d.offset().left) / d.width() < b.options.imageClickDivision ? b.prev(c) : b.next(c)
        ...
        if (!this.isShown && this.jQueryelement.hasClass("modal-gallery")) {
            var b = this.jQueryelement, d = this.options, e = a(window).width(), f = a(window).height();
            b.hasClass("modal-fullscreen") ? (this._loadImageOptions = {maxWidth: e, maxHeight: f, canvas: d.canvas}, b.hasClass("modal-fullscreen-stretch") && (this._loadImageOptions.minWidth = e, this._loadImageOptions.minHeight = f)) : this._loadImageOptions = {maxWidth: e - d.offsetWidth, maxHeight: f - d.offsetHeight, canvas: d.canvas}, e > 480 && b.css({"margin-top": -(b.outerHeight() / 2), "margin-left": -(b.outerWidth() / 2)}), this.initGalleryEvents(), this.initLinks(), this.jQuerylinks.length && (b.find(".modal-slideshow, .modal-prev, .modal-next").toggle(this.jQuerylinks.length !== 1), b.toggleClass("modal-single", this.jQuerylinks.length === 1), this.loadImage())
        }
        c.apply(this, arguments)
    }, hide: function () {
        this.isShown && this.jQueryelement.hasClass("modal-gallery") && (this.options.delegate = document, this.options.href = null, this.destroyGalleryEvents()), d.apply(this, arguments)
    }}), a(function () {
