...
class OrganizationNameField(forms.CharField):
    def validate(self, value):
        super(forms.CharField, self).validate(value)
        organization = Organization.objects.filter(name=value)
        if organization:
            raise forms.ValidationError('The group name "' + value + '" name is already in use.')

#Credits goes to http://djangosnippets.org/snippets/2753/
